...
- B型肝炎治療ガイドライン第5版: 慢性肝炎・肝硬変の治療対象条件、急性肝炎・急性肝不全、再活性化対策
- C型肝炎治療ガイドライン第8.4版: 初回DAA候補、腎機能・肝予備能との対応、再治療・SVR後フォロー
- PC 2ペイン、スマートフォン1列・入力全体の折り畳み
- 架空デモ症例（自動保存なし）

RECAM-J、ICI肝障害の臨床ロジック、DILI因果関係スコア、肝移植適応スコアの自動計算・適応の自動判定は現段階の対象外です。
...
- B型肝炎治療ガイドライン第5版（2026年6月）
- C型肝炎治療ガイドライン第8.4版（2025年4月）
- ガイドライン確認日: 2026-08-15

オフライン版は最新情報へ自動追随しません。FIB-3は研究指標として表示し、ガイドライン推奨またはカットオフ判定を表示しません。Lilleは計算値だけを表示し、治療継続・中止を自動判定しません。

確認済み記載と出典階層は `src/clinical-sources.json` に保持しています。臨床計算はUIから分離し、既存の `src/scores.js` と追加の純粋関数をテストしています。
