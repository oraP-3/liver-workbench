...
test("domestic steroid recommendation requires clinician-selected ACLF context", () => {
  const unselected = buildAldGuidelineReference({
    clinicianSelectedContext: "unselected",
    mdf: 40,
  });
  assert.equal(
    unselected.cards.some(
      (item) => item.id === "domestic-steroid-consideration",
    ),
    false,
  );

  const selected = buildAldGuidelineReference({
    clinicianSelectedContext: "aclf-with-severe-alcoholic-hepatitis",
    mdf: 40,
  });
  const recommendation = selected.cards.find(
    (item) => item.id === "domestic-steroid-consideration",
  );
  assert.equal(
    recommendation.sourceLevel,
...
test("reported GMA evidence is not labeled as a domestic recommendation", () => {
  const result = buildAldGuidelineReference({
    clinicianSelectedContext: "severe-alcoholic-hepatitis",
  });
  const report = result.cards.find(
    (item) => item.id === "steroid-pulse-gma-report",
  );
...
test("complication text only includes clinician-selected complications", () => {
  const result = buildAldGuidelineReference({
    complications: {
      ...
      dic: false,
      gastrointestinalBleeding: true,
    },
  });
  const narrative = result.cards.find(
    (item) => item.id === "complication-treatment-narrative",
  );
