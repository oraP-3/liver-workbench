...
test("relative-only assessment is retained without a baseline date", () => {
  const record = addAssessment(
    createCase({ caseCode: "X-1" }),
    createAssessment({ relativeDay: 3 }),
  );
...
test("medication timing is synchronized to case baseline", () => {
  const record = addMedication(
    createCase({ baselineDate: "2026-08-01" }),
    createMedicationExposure({
      name: "被疑薬A",
      startRelativeDay: -10,
      stopDate: "2026-08-03",
...
test("legacy v1 mock case migrates into first longitudinal assessment", () => {
  const migrated = migrateLegacyCase({
    data: {
      caseId: "AIH-024",
      age: "58",
      sex: "女性",
