...
test("FIB-4 calculation", () => {
  assert.ok(
    Math.abs(fib4({ age: 60, ast: 40, alt: 36, platelets104: 20 }) - 2) < 1e-12,
  );
});
...
test("FIB-3 calculation", () => {
  ...
  assert.ok(
    Math.abs(fib3({ ast: 40, alt: 36, platelets104: 20 }) - expected) < 1e-12,
  );
});

test("Child-Pugh classification", () => {
  const result = childPugh({
    bilirubin: 2.5,
    albumin: 3,
    inr: 1.8,
    ascites: "controlled",
  ...
  assert.deepEqual(
    { score: result.score, class: result.class },
    { score: 9, class: "B" },
  );
});
...
test("Lille score is bounded", () => {
  const result = lille({
    age: 50,
    albuminDay0GPerL: 30,
    bilirubinDay0UmolL: 250,
    bilirubinDay7UmolL: 180,
    prothrombinTimeSeconds: 18,
  ...
  assert.ok(
    Math.abs(
      maddreyDiscriminantFunction({
        ...
        controlProthrombinTimeSeconds: 12,
        totalBilirubin: 10,
      }) - 46.8,
    ) < 1e-12,
  );
...
test("JAS classifies severe alcoholic hepatitis using INR", () => {
  const result = japanAlcoholicHepatitisScore({
    whiteBloodCellCount: 22000,
    creatinine: 3.2,
    inr: 2.1,
    totalBilirubin: 12,
    gastrointestinalBleedingOrDic: true,
    age: 52,
  });
  assert.deepEqual(
    {
      ...
      coagulationInput: result.coagulationInput,
    },
    { score: 16, severity: "severe", coagulationInput: "inr" },
  );
});
...
test("JAS uses prothrombin activity when INR is unavailable", () => {
  const result = japanAlcoholicHepatitisScore({
    ...
    creatinine: 1.2,
    prothrombinActivity: 35,
    totalBilirubin: 4,
    gastrointestinalBleedingOrDic: false,
    age: 45,
  });
  assert.deepEqual(
    {
    ...
    { score: 7, severity: "mild", coagulationInput: "prothrombinActivity" },
  );
});
