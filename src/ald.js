...
export function buildAldGuidelineReference({
    ...
    cards.push(
      card(
        ...
        37,
        {
          ...
          citedThreshold: 32,
          provenanceNote:
            "国内Recommendation欄の直接推奨ではなく、国内ガイド本文中で引用されたAASLDガイドラインの記載",
        },
      ),
    );
  }
  ...
  const selectedComplications = complicationEntries.filter(
    ([key]) => complications[key] === true,
  );
  ...
  if (
    ...
    cards.push(
      card(
        ...
        {
          evidenceNote:
            "本文では少数例の報告として扱われ、アルコール性肝炎に対する顆粒球単球吸着除去療法は保険適用外と記載されている。",
        },
      ),
    );
    cards.push(
  ...
  return {
    ...
    cards,
    disclaimer:
      "入力値とガイドライン記載の対応を示す参照情報であり、病態の自動判定または治療指示ではない。",
  };
}
